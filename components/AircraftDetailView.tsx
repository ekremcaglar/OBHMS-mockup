  ...
  return (
    <div className="space-y-6 print-container">
       <div className="hidden print-show mb-4">
           <h1 className="text-3xl font-bold">{t('aircraft_health_report')}</h1>
           <p className="text-lg">{t('tail_number')}: {aircraft.tailNumber}</p>
           <p className="text-sm">{t('report_generated')}: {new Date().toLocaleString()}</p>
       </div>
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
            <button onClick={onBack} className="p-2 rounded-full hover:bg-gray-800 no-print">
        ...
        <div className="flex items-center space-x-6 text-sm">
             <button onClick={handlePrint} className="flex items-center space-x-2 text-sm font-semibold py-2 px-4 rounded-lg transition-colors bg-slate-600 hover:bg-slate-500 text-white no-print">
            ...
            <div><span className="text-gray-400">{t('flight_hours')}: </span><span className="font-semibold text-white">{aircraft.flightHours}</span></div>
            <div><span className="text-gray-400">{t('mc_rate')}: </span><span className="font-semibold text-white">{aircraft.missionCapableRate}%</span></div>
        </div>
      </div>

      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 shadow-lg backdrop-blur-sm">
        <h2 className="text-xl font-semibold text-white mb-4">{t('system_health_overview')}</h2>
                ...
                <div key={system.id} className={`p-4 rounded-lg border-2 ${system.status === 'critical' ? 'border-red-500 bg-red-500/10' : system.status === 'warning' ? 'border-yellow-400 bg-yellow-400/10' : 'border-gray-600 bg-gray-700/30'}`}>
                    ...
                    {system.metrics.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-gray-600 space-y-3">
                            <h4 className="text-xs font-bold text-gray-400 uppercase">{t('key_metrics')}</h4>
