...
const FrequencyAnalysis: React.FC = () => {
    const { t } = useI18n();
    const [dataSourceId, setDataSourceId] = useState<string | null>('engine-vibration-fft');
    const [xAxisField, setXAxisField] = useState<string | null>('frequency');
    const [yAxisFields, setYAxisFields] = useState<string[]>(['amplitude']);

    const frequencyDataSources = useMemo(() =>
        CHART_DATA_SOURCES.filter(ds => ds.id.includes('fft'))
    , []);
            ...
            <aside className="lg:col-span-3 bg-white dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/30 rounded-xl p-6 flex flex-col">
                ...
                <div className="space-y-6">
                    <FormSelect label={t('data_source')} value={dataSourceId || ''} onChange={e => handleDataSourceChange(e.target.value)}>
                        ...
                        {frequencyDataSources.map(ds => <option key={ds.id} value={ds.id}>{ds.name}</option>)}
                    </FormSelect>
